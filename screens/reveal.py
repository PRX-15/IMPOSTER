...
class RevealScreen(Screen):
    def __init__(self, state, **kwargs):
        ...
        self.root = BoxLayout(orientation="vertical", padding=[dp(24), dp(36)], spacing=dp(16))
    ...
    def reveal(self, *_):
        ...
        if info["is_imposter"]:
            self.card.text = f"[size=18sp]YOU ARE THE IMPOSTER[/size]\n\n[size=16sp]CATEGORY[/size]\n[size=34sp]{info['category']}[/size]"
        else:
            self.card.text = f"[size=16sp]WORD[/size]\n[size=36sp]{info['word']}[/size]\n\n[size=16sp]CATEGORY[/size]\n[size=34sp]{info['category']}[/size]"
