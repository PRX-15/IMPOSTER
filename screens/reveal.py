...
class EntranceLabel(NeonLabel):
    ...
    def __init__(self, **kwargs):
        ...
        with self.canvas.before:
            PushMatrix()
            self._entrance_translate = Translate(0, 0, 0)
        with self.canvas.after:
            PopMatrix()
        self.bind(entrance_offset=self._sync_entrance_offset)

    def _sync_entrance_offset(self, *_):
        self._entrance_translate.y = self.entrance_offset
...
class RoundedStencilView(Widget):
    ...
    def __init__(self, radius=28, **kwargs):
        ...
        with self.canvas.before:
            StencilPush()
            self._stencil_mask = RoundedRectangle(pos=self.pos, size=self.size, radius=[self.clip_radius])
            StencilUse()
        with self.canvas.after:
            StencilUnUse()
            self._stencil_clear = RoundedRectangle(pos=self.pos, size=self.size, radius=[self.clip_radius])
            StencilPop()
...
class PlainCurtain(Button):
    """A flat moving cover. The viewport supplies the rounded clipping."""
...
class RevealScreen(Screen):
    ...
    def _prepare_entrance_widget(self, widget):
        widget.opacity = 0
        widget.entrance_offset = dp(28)
        self.entrance_widgets.append(widget)
        return widget

    def start_entrance_animation(self):
        for widget in self.entrance_widgets:
            Animation.cancel_all(widget, "opacity", "entrance_offset")
            Animation(opacity=1, entrance_offset=0, duration=0.45, t="out_cubic").start(widget)
        if hasattr(self, "card_area"):
            Animation.cancel_all(self.card_area, "opacity")
            Animation(opacity=1, duration=0.45, t="out_cubic").start(self.card_area)
    ...
    def _draw_card_border(self, *_):
        self.card_border.rounded_rectangle = [self.card_area.x, self.card_area.y, self.card_area.width, self.card_area.height, dp(28)]
    ...
    def _slide_curtain_up(self):
        ...
        self.curtain_animation = Animation(y=self.card_area.top, duration=0.38, t="in_out_cubic")
    ...
    def _slide_curtain_down(self):
        self._stop_curtain_animation()
        self.curtain_animation = Animation(y=self.card_area.y, duration=0.38, t="in_out_cubic")
        self.curtain_animation.bind(on_complete=self._clear_curtain_animation)
        self.curtain_animation.start(self.curtain)
    ...
    def build_turn(self):
        ...
        self.player_label = self._prepare_entrance_widget(EntranceLabel(text=self.state.players[i].upper(), font_size="30sp", bold=True, size_hint_y=.14))
        self.root.add_widget(self.player_label)
        self.turn_label = self._prepare_entrance_widget(EntranceLabel(text="It's your turn.", font_size="18sp", color=COLORS["muted"], size_hint_y=.07))
        ...
        with self.card_background.canvas.before:
            Color(*COLORS["card"])
            self.card_background_rect = Rectangle(pos=self.card_background.pos, size=self.card_background.size)
        ...
        self.secret_label = NeonLabel(text="", markup=True, size_hint=(None, None), font_size="20sp", color=COLORS["text"])
        self.card_area.add_widget(self.secret_label)
        self.curtain = PlainCurtain(text="TAP HERE TO\nREVEAL YOUR ROLE", font_size="23sp", size_hint=(None, None))
    ...
    def reveal(self, *_):
        ...
        category_size = self._adaptive_size(info["category"], normal=30, compact=26, minimum=20, compact_at=14, minimum_at=30)
        category_hi_size = self._adaptive_size(info["category_hi"], normal=20, compact=18, minimum=15, compact_at=12, minimum_at=26)
        if info["is_imposter"]:
            ...
            word_size = self._adaptive_size(info["word"], normal=34, compact=29, minimum=20, compact_at=12, minimum_at=28)
            word_hi_size = self._adaptive_size(info["word_hi"], normal=21, compact=18, minimum=15, compact_at=12, minimum_at=26)
        ...
        self.next_btn.text = "FINISH REVEAL" if self.state.current_reveal_index == len(self.state.players) - 1 else f"PASS THE PHONE TO {self.state.players[self.state.current_reveal_index + 1].upper()}"
