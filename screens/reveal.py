    ...
    def __init__(self, radius=28, **kwargs):
        ...
        with self.canvas.before:
            ...
            self._stencil_mask = RoundedRectangle(
                pos=self.pos, size=self.size, radius=[self.clip_radius]
            )
            StencilUse()
        with self.canvas.after:
            StencilUnUse()
...
class PlainCurtain(Button):
    """Flat moving cover; the parent viewport provides rounded clipping."""
...
class RevealScreen(Screen):
    ...
    def build_turn(self):
        ...
        self.player_label = self._prepare_entrance_widget(
            EntranceLabel(
        ...
        self.turn_label = self._prepare_entrance_widget(
            EntranceLabel(
                text="It's your turn.",
                font_size="18sp",
                color=COLORS["muted"],
        ...
        self.card_background.bind(
            pos=self._sync_card_background,
            size=self._sync_card_background,
        )
        ...
        self.card_border_overlay.bind(
            pos=self._draw_card_border,
            size=self._draw_card_border,
        )
        self.card_area.add_widget(self.card_border_overlay)
        self.card_area.bind(
            pos=self._sync_card_geometry,
            size=self._sync_card_geometry,
        )
        ...
        self.privacy_label = self._prepare_entrance_widget(
            EntranceLabel(
    ...
    def _sync_card_geometry(self, *_):
        ...
        self.curtain.pos = (x, y if not self.secret_visible else self.curtain.y)
    ...
    def reveal(self, *_):
        ...
        Animation(opacity=1, duration=0.28, t="out_cubic").start(self.next_btn)
