...
class PlayerRow(BoxLayout):
    def __init__(self, number, delete_callback=None, **kwargs):
        ...
        self.add_widget(
            Image(
                source=asset_path("main-menu", "player-icon.png"),
                size_hint_x=None,
                width=dp(34),
            )
        )
        ...
        self.add_widget(
            Image(
                source=asset_path("main-menu", "pencil-icon.png"),
                size_hint_x=None,
                width=dp(26),
            )
        )
        ...
        self.delete_btn = RoundedButton(
            ...
            width=dp(64),
            height=dp(34),
            bg_color=COLORS.get("danger", (0.85, 0.08, 0.28, 1)),
        )
...
class MainMenuScreen(Screen):
    def __init__(self, state, **kwargs):
        ...
        stack.add_widget(
            NeonLabel(
                text="Pass the phone. Find the fake.",
                font_size="14sp",
                color=COLORS["muted"],
                size_hint_y=.15,
            )
    ...
    def add_player(self, *_):
        ...
        row = PlayerRow(
            len(self.rows) + 1,
            delete_callback=self.remove_player,
        )
    ...
    def remove_player(self, row):
        # Never allow the game to have fewer than 3 players.
        if len(self.rows) <= 3:
            return
        ...
        # Keep player numbers sequential after a deletion.
    ...
    def _refresh_player_controls(self):
        ...
        # Delete is available only when removing a player would still leave
        # at least the required minimum of 3 players.
        can_delete = count > 3
